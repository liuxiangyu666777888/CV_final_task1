...
class NeRFRenderer(nn.Module):
    ...
    def export_mesh(self, path, resolution=None, decimate_target=-1, S=128):
        ...
        # DMTet already outputs a valid triangle mesh. Native clean/decimate steps
        # are skipped here because they can segfault in this environment.
        ...
        def _export(v, f, tile_size=8, tile_pad=1, name=''):
            # Native UV baking via xatlas/nvdiffrast is unstable in this setup.
            # Use a per-face atlas instead: each triangle gets its own solid-color tile.
            v_np = v.detach().cpu().numpy()
            f_np = f.detach().cpu().numpy()

            logger.info(
                f'[INFO] exporting safe textured mesh: v={v_np.shape} f={f_np.shape}')

            vertex_albedo = []
            head = 0
            while head < v.shape[0]:
                tail = min(head + 65536, v.shape[0])
                results_ = self.density(v[head:tail])
                vertex_albedo.append(results_['albedo'].float().clamp(0, 1))
                head = tail

            vertex_albedo = torch.cat(vertex_albedo, dim=0)
            face_colors = vertex_albedo[f.long()].mean(dim=1)
            face_colors = (face_colors.detach().cpu().numpy() * 255).astype(np.uint8)

            num_faces = max(int(f_np.shape[0]), 1)
            ...
            vt_np = np.zeros((num_faces * 3, 2), dtype=np.float32)
            ft_np = np.arange(num_faces * 3, dtype=np.int32).reshape(num_faces, 3)
            ...
            for i in range(f_np.shape[0]):
                ...
                vt_np[3 * i + 0] = [(px + x0_off) / atlas_w, 1.0 - (py + y0_off) / atlas_h]
                vt_np[3 * i + 1] = [(px + x1_off) / atlas_w, 1.0 - (py + y0_off) / atlas_h]
                vt_np[3 * i + 2] = [(px + x0_off) / atlas_w, 1.0 - (py + y1_off) / atlas_h]

            texture_file = os.path.join(path, f'{name}texture_kd.png')
            legacy_texture_file = os.path.join(path, f'{name}albedo.png')
            obj_file = os.path.join(path, f'{name}mesh.obj')
            mtl_file = os.path.join(path, f'{name}mesh.mtl')

            logger.info(f'[INFO] writing texture atlas to {texture_file}')
            cv2.imwrite(texture_file, cv2.cvtColor(texture, cv2.COLOR_RGB2BGR))
            cv2.imwrite(legacy_texture_file, cv2.cvtColor(texture, cv2.COLOR_RGB2BGR))
            ...
            with open(mtl_file, "w") as fp:
                ...
                fp.write(f'map_Kd {name}texture_kd.png \n')
